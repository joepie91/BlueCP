...
<body>
    ...
    <div class="loginform">
		...
		<form id="form1" name="form1" method="post" action="index.php?id=login">
		  ...
		  {%if ForgotPasswordEnabled == enabled}<a href="forgot.php">Forgot Password?</a>{%/if}
		  {%if RegistrationEnabled == enabled}{%if ForgotPasswordEnabled == enabled} | {%/if}{%/if}
		  {%if RegistrationEnabled == enabled}<a href="register.php">Register</a>{%/if}</div>
        </div>
    </div>
